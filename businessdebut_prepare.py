#!/usr/bin/env python3
"""
Business Debut Batch Preparer
Reads businessdebut_latest.json, fetches article text, and creates
ready-to-paste batch files (prompt already included at the top).

Usage:
    python businessdebut_prepare.py

Output:
    bizdbatch_1.txt, bizdbatch_2.txt, ... (each ready to paste directly into Claude)
"""
...
def main():
    src_path = Path("businessdebut_latest.json")
    if not src_path.exists():
        print("❌  businessdebut_latest.json not found. Run businessdebut_scraper.py first.")
        return
    ...
    for i, fname in enumerate(batch_files):
        ...
        print(f"    3. Run: python businessdebut_extraction_parser.py {out_name}{flag}")

    print("\n  After all batches:")
    print("    git add businessdebut_extraction_latest.json")
    print("    git commit -m \"chore: business debut extraction update\"")
    print("    git push")
    print("=" * 60)
